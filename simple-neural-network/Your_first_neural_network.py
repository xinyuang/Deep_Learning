...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            hidden_inputs = np.dot(X[None,:],self.weights_input_to_hidden)     #None # signals into hidden layer
            ...
            hidden_error = np.dot(  output_error_term,np.transpose(self.weights_hidden_to_output)) #None
            ...
            delta_weights_i_h +=   (hidden_error_term)*np.transpose(X[None,:]) #None
            #delta_weights_i_h +=   (hidden_error_term)*X[:,None])
            # Weight step (hidden to output)
            delta_weights_h_o += output_error_term *np.transpose(hidden_outputs)  #None
