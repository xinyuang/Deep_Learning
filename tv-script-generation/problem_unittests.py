...
def test_build_rnn(build_rnn):
    with tf.Graph().as_default():
        ...
        cell = tf.contrib.rnn.MultiRNNCell([tf.contrib.rnn.BasicLSTMCell(test_rnn_size) for i in range(test_rnn_layer_size)])
        ...
        outputs, final_state = build_rnn(cell, test_inputs)
        ...
        assert final_state.get_shape().as_list() == [test_rnn_layer_size, 2, -1, test_rnn_size],\
...
def test_build_nn(build_nn):
    with tf.Graph().as_default():
        ...
        test_cell = tf.contrib.rnn.MultiRNNCell([tf.contrib.rnn.BasicLSTMCell(test_rnn_size) for i in range(test_rnn_layer_size)])
        #test_cell = rnn.MultiRNNCell([rnn.BasicLSTMCell(test_rnn_size)] * test_rnn_layer_size)
