...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob, 
    ...
    """
    # TODO: Implement Function
    def new_cell(rnn_size):
        lstm_cell = tf.contrib.rnn.LSTMCell(rnn_size,initializer=tf.random_uniform_initializer(-0.1,0.1,seed=2))
        drop = tf.contrib.rnn.DropoutWrapper(lstm_cell, output_keep_prob=keep_prob)
        return drop
    cell = tf.contrib.rnn.MultiRNNCell([new_cell(rnn_size) for i in range(num_layers)])
    
    embed = tf.contrib.layers.embed_sequence(rnn_inputs, source_vocab_size, encoding_embedding_size)
    output, rnn_state = tf.nn.dynamic_rnn(cell, embed, dtype=tf.float32)
...
def decoding_layer(dec_input, encoder_state,
    ...
    dec_cell = tf.contrib.rnn.MultiRNNCell([new_cell(rnn_size) for i in range(num_layers)])
...
def sentence_to_seq(sentence, vocab_to_int):
    ...
    """
    # TODO: Implement Function
    sentences_to_seq = [vocab_to_int.get(word, vocab_to_int.get('<UNK>')) for word in sentence.lower().split()]
    return sentences_to_seq
